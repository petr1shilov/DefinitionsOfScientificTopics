...
class OEСDApi:
    ...
    def get_oecd(self, answer):
        df_updated = pd.DataFrame([
            [outer_key, inner_key, value]
                ...
                for value in values 
                    if not self.oecd_df[self.oecd_df['3 уровень Русское наименование'].str.lower() == value.lower()].empty
    ...
    def dict_to_excel(self, path):
        try:
            ...
            api_logger.info('Лабораторя успешно прочитала')
        except:
            df = pd.DataFrame()
            api_logger.info('read df error')
